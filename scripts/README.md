...
| `check-pdf-determinism.mjs / check-pdf-fonts.mjs` | `@myriaddreamin/typst-ts-node-compiler`, built packages | The PDF renderer's output is byte-identical across **separate processes**, carries no wall-clock date, and reports what it cannot express (ADR-0003) |
...
| `check-template-strict.mjs` | built packages | What the three shipped templates do under `--strict`: all exit 2 for constructs Markdown cannot express, and a control document must exit 0 so a constant exit code fails. Was inline shell in `ci.yml`, where gate parity could not see it |
