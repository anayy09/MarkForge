...
| docx/manuscript-footnotes-equations.docx | generated | Apache-2.0 | MarkForge | `../scripts/build-corpus-fixtures.mjs` | §2.2: three footnotes with Word's separator entries, a display OMML equation with a caption, and an inline OMML equation. The inline one is the case the phrasing walk dropped silently for five phases |
| docx/manuscript-endnotes-crossrefs.docx | generated | Apache-2.0 | MarkForge | `../scripts/build-corpus-fixtures.mjs` | §2.2: endnotes rather than footnotes, plus a REF field cross-reference whose cached result is the visible text and whose instruction is the thing worth recovering |
| docx/tables-merged-horizontal.docx | generated | Apache-2.0 | MarkForge | `../scripts/build-corpus-fixtures.mjs` | §2.5: w:gridSpan across two columns, a two-row header GFM cannot express, and a full-width merged divider row |
| docx/tables-merged-vertical.docx | generated | Apache-2.0 | MarkForge | `../scripts/build-corpus-fixtures.mjs` | §2.5: w:vMerge restart and continue over three rows, plus a header column. The continuation cells are the ones a naive reader emits as empty |
| docx/tables-block-content.docx | generated | Apache-2.0 | MarkForge | `../scripts/build-corpus-fixtures.mjs` | §2.5: a cell holding three paragraphs and a numbered list, and a cell holding a nested table. SPEC §2.7.1 widened TableCell.children for exactly this |
| docx/tables-merged-combined.docx | generated | Apache-2.0 | MarkForge | `../scripts/build-corpus-fixtures.mjs` | §2.5: horizontal and vertical merges, a two-row header, and a spanning footer row in one table, so a score change names the construct that moved |
| docx/tracked-changes-single-author.docx | generated | Apache-2.0 | MarkForge | `../scripts/build-corpus-fixtures.mjs` | §2.12: w:ins and w:del from one author, both inline and as whole paragraphs. The simple case a reader must get right before the overlapping one |
| docx/tracked-changes-two-authors.docx | generated | Apache-2.0 | MarkForge | `../scripts/build-corpus-fixtures.mjs` | §2.12: overlapping revisions from two authors in one sentence, the case CORPUS §2.12 names as where a range-based revision model corrupts and SPEC §2.3's wrapping nodes earn their keep |
| docx/comments-anchored.docx | generated | Apache-2.0 | MarkForge | `../scripts/build-corpus-fixtures.mjs` | §2.12: two comments with anchor ranges, one spanning several words and one a whole sentence. All three anchor elements sat in the adapter's property-element list until 2026-08-01, so comments were discarded silently |
| md/flavor-probe.md | authored | Apache-2.0 | MarkForge | — | §2.13: the construct-dense probe for the seven SPEC §4.1 flavour presets — front matter, a footnote, inline and display math, an admonition, and a table. That set is what separates all seven on output bytes |
| md/rtl-arabic.md | authored | Apache-2.0 | MarkForge | — | §2.10 **PROVISIONAL — no native-speaker review.** Arabic RTL with embedded Latin to force a bidi transition, Arabic-Indic against Latin digits, and Arabic punctuation. Trusted for byte-preservation and grapheme counting only, not as a judgement that the prose is idiomatic |
| md/rtl-hebrew.md | authored | Apache-2.0 | MarkForge | — | §2.10 **PROVISIONAL — no native-speaker review.** Hebrew RTL with embedded Latin, plus niqqud (combining marks) and Hebrew gershayim. Same trust boundary as the Arabic file |
| md/cjk-japanese.md | authored | Apache-2.0 | MarkForge | — | §2.10 **PROVISIONAL — no native-speaker review.** Japanese with full-width punctuation against half-width, kagi and double-kagi brackets, wave dash and ellipsis, and hiragana/katakana/kanji/Latin mixing. No spaces between words, which is the line-breaking case |
| md/cjk-chinese.md | authored | Apache-2.0 | MarkForge | — | §2.10 **PROVISIONAL — no native-speaker review.** Simplified Chinese with full-width punctuation, book-title marks, enumeration comma, and a simplified/traditional contrast pair. Same trust boundary |
